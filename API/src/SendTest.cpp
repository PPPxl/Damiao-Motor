...
float k = 0.0;
motorSend.MIT_ctrl_motor(0, 0, 0.5, 0.01, 0);
motorSend.MIT_ctrl_motor(0, 0, 0.5, 0.01, 0);
usleep(1.5e6);
...
while(1)
  {
    ...
    ofs4 <<timeUse1<<endl;
    usleep(4e2);
    motorSend.MIT_ctrl_motor(pos2, vel2, 1, 0.1, 0);
...
while(1)
{
  int updatedID = motorRecv.motor_state_receive();
  gettimeofday(&endTime,NULL);  
  timeUse = 1e6*(endTime.tv_sec - startTime.tv_sec) + endTime.tv_usec - startTime.tv_usec;
  ofs1 <<updatedID<<","<<timeUse<<","<<motorRecv.present_position[updatedID-1]<<endl;
  ofs2 <<updatedID<<","<<timeUse<<","<<motorRecv.present_velocity[updatedID-1]<<endl;
  ofs3 <<updatedID<<","<<timeUse<<","<<motorRecv.present_torque[updatedID-1]<<endl;
}
ofs1.close();  
ofs2.close();  
ofs3.close(); 
